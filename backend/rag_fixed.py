...
from qdrant_client_custom import qdrant_manager

load_dotenv()

# Configure Gemini API only (as requested, no OpenAI)
...
class RAGService:
    ...
    async def generate_response(self, query: str, context: List[Dict]) -> str:
        """
        Generate a response using only Gemini based on the query and context
        """
        ...
        try:
            # Use only Gemini (no OpenAI fallback as requested)
    ...
    async def generate_response_from_selected_text(self, query: str, selected_text: str) -> str:
        """
        Generate a response using only the selected text with Gemini
        """
        ...
        try:
            ...
            print(f"Error generating response from selected text with Gemini: {e}")
